...
\def\Abstract{%
Image outpainting is a task in the area of generative artificial intelligence, where the goal is to expand an image beyond its borders in a feasible way. The goal of this work is to create an AI capable of sky image outpainting by implementing several new techniques in the field. We train three models, a tokenizer for converting images to tokens and back, a masked generative transformer for performing outpainting on tokens and a super sampler for upscaling the result, all on a dataset of sky images. Then, we propose an algorithm that combines the trained models to solve the outpainting task. We describe the results of training each model and those of the final algorithm; Our contribution consists mainly in providing a working, open-source implementation including the trained models capable of outpainting.
}

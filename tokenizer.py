...
class VQVAEModel:
    ...
    @staticmethod
    def new(args):
        return VQVAEModel(args.img_size, args.filters, args.residual_layer_multipliers, args.num_res_blocks, args.embedding_dim, args)

    @staticmethod
    def load(fname):
        vqvae = VQVAEModel.__new__(VQVAEModel)
        vqvae._model = tf.keras.models.load_model(fname, {"VectorQuantizer":VectorQuantizer, "scaled_mse_loss":scaled_mse_loss})
    ...
    def decode(self, tokens) -> tf.Tensor:
        return self._decode_model(self._quantizer.get_embedding(tokens)) #type: ignore
